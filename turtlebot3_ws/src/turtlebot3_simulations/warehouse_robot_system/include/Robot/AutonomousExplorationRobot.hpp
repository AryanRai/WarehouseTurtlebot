...
class AutonomousExplorationRobot
{
    ...
    private:
        ...
        bool in_recovery_;
        int recovery_attempt_; // Track which recovery attempt (0=forward,
                               // 1=backward, etc.)
        int consecutive_no_frontiers_count_; // Track how many times we found no
                                             // frontiers
        ...
        // attempts before recovery (reduced to trigger faster when stuck)
        static constexpr int MAX_NO_PATH_BEFORE_RECOVERY = 3;
        static constexpr double RECOVERY_DURATION = 3.0; // seconds to rotate during recovery
        
        // max recovery attempts before giving up (balanced for thorough exploration)
        static constexpr int MAX_RECOVERY_ATTEMPTS = 10;
        ...
        bool home_path_completed_; // Track if path to home is complete (like
                                   // delivery robot)
        ...
        double distance_before_recovery_; // Distance to home before recovery
                                          // started
        ...
        void returnToHome();
        void performAdvancedReturnHomeRecovery(
            const geometry_msgs::msg::Pose &current_pose,
            double current_distance);
        void preciseDocking(const geometry_msgs::msg::Pose &current_pose,
                            double distance_to_home);
        bool isObstacleAhead(double min_distance =
                            0.3); // Check if obstacle within distance ahead
        bool isObstacleBehind(double min_distance =
                             0.3); // Check if obstacle within distance behind
        bool hasLineOfSight(const geometry_msgs::msg::Point &from,
                            const geometry_msgs::msg::Point &to,
                            const nav_msgs::msg::OccupancyGrid
                                &map); // Check clear path with robot footprint

        // Docking thresholds

        // Enter docking mode within 15cm (reduced to avoid wall collisions)
        static constexpr double DOCKING_DISTANCE = 0.15;
