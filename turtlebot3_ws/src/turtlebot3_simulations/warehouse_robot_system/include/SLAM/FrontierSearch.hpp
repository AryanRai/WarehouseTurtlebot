...
class FrontierSearch
{
    public:
        ...
        struct Frontier
        {
            ...
            Frontier() : size(0) {}
            Frontier(uint32_t s, const geometry_msgs::msg::Point &c)
                : size(s), centroid(c)
            {
            }
        };
        ...
        // Hash functor for GridCell (std::pair<int, int>) to use in
        // unordered_map
        struct GridCellHash
        {
        ...
        static std::pair<FrontierList, std::vector<GridCell>>
        search(const nav_msgs::msg::OccupancyGrid &mapdata,
               const GridCell &start, bool include_frontier_cells = false);
    ...
    private:
        ...
        static bool
        isNewFrontierCell(const nav_msgs::msg::OccupancyGrid &mapdata,
                          const GridCell &cell,
                          const std::unordered_map<GridCell, bool, GridCellHash>
                              &is_frontier);
