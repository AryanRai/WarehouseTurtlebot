...
DeliveryRobot::DeliveryRobot(rclcpp::Node::SharedPtr node)
    : WarehouseRobot(node), is_delivering_(false), current_delivery_index_(0)
{
    ...
    if (!check.good())
    {
        ...
        log << "Timestamp,RequestID,FromZone,ToZone,Distance(m),Time(s),Status,"
               "Notes\n";
        log.close();
    }
    ...
    try
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "Loaded %zu delivery zones",
                    zones_.size());
    }
    catch (const std::exception &e)
    {
        RCLCPP_WARN(node_->get_logger(),
                    "No existing zones file, starting fresh");
    }

    RCLCPP_INFO(node_->get_logger(), "Delivery Robot initialized");
    RCLCPP_INFO(node_->get_logger(), "Using %zu pre-defined delivery zones",
                zones_.size());
    RCLCPP_INFO(node_->get_logger(),
                "Use 'Define Delivery Zones' mode to add/edit zones");
}
...
void DeliveryRobot::saveZonesToFile(const std::string &filename)
{
    ...
    RCLCPP_INFO(node_->get_logger(), "Saved %zu zones to %s", zones_.size(),
                filename.c_str());
}
...
void DeliveryRobot::clearZones() { zones_.clear(); }

// ============================================================================
// Delivery Management
// ============================================================================

void DeliveryRobot::addDeliveryRequest(
    const DeliveryData::DeliveryRequest &request)
{
    delivery_queue_.push_back(request);
    RCLCPP_INFO(node_->get_logger(), "Added delivery: %s -> %s (Priority: %d)",
                request.from_zone.c_str(), request.to_zone.c_str(),
                request.priority);
}

void DeliveryRobot::clearDeliveryQueue() { delivery_queue_.clear(); }
...
void DeliveryRobot::startDeliveries()
{
    ...
    if (use_tsp_optimization_)
    {
        ...
        optimized_route_ =
            optimizeRouteTSP(current_pose.position, delivery_queue_);
    }
    else
    {
        RCLCPP_INFO(node_->get_logger(), "Using ordered delivery sequence");
        optimized_route_ =
            optimizeRoute(current_pose.position, delivery_queue_);
    }

    RCLCPP_INFO(node_->get_logger(),
                "Starting deliveries with %s route of %zu stops",
    ...
    const std::vector<DeliveryData::DeliveryRequest> &requests)
{
    ...
    auto optimal_tour = simulatedAnnealing(
        distance_matrix, 0); // Start from index 0 (start position)
...
void DeliveryRobot::update()
{
    ...
    // Check if we've reached the zone with tight tolerance OR if path is
    // complete and we're close
    bool zone_reached =
        (distance_to_zone < TARGET_TOLERANCE) ||
    ...
    if (zone_reached)
    {
        ...
        record.from_zone =
            current_delivery_index_ > 0
                ? optimized_route_[current_delivery_index_ - 1].name
                : "Start";
    ...
    if (distance_to_zone < 1.0)
    {
        ...
        bool has_clear_path = hasLineOfSight(
            current_pose.position, current_zone.position, *current_map);
        ...
        if (has_clear_path)
        {
            ...
            {
                RCLCPP_INFO(node_->get_logger(),
                            "Entering precise zone docking mode (%.3fm from "
                            "%s, clear path)",
            ...
            preciseTargetDocking(current_pose, current_zone.position,
                                 distance_to_zone, current_zone.name);
            return;
        }
        else
        {
            RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
                                 "Close to zone (%.2fm) but obstacle detected "
    ...
    if (path_completed_)
    {
        ...
        {
            RCLCPP_INFO(node_->get_logger(),
                        "Path complete, %.2fm from zone with clear line of "
        ...
        else if (!has_clear_path)
        {
            ...
            RCLCPP_INFO(node_->get_logger(),
                        "Path complete, %.2fm from zone but obstacle detected "
                        "- accepting position",
                        distance_to_zone);
            ...
            record.success = true;
            record.notes =
                "Reached nearest accessible point (obstacle at exact location)";
        ...
        else
        {
            ...
            RCLCPP_WARN(node_->get_logger(),
                        "Path complete but %.2fm from zone (too far for "
    ...
    if (!motion_controller_->hasPath())
    {
        RCLCPP_INFO(node_->get_logger(), "Planning path to zone: %s",
        ...
        GridCell goal =
            PathPlanner::worldToGrid(*current_map, current_zone.position);
        ...
        {
            RCLCPP_WARN(node_->get_logger(),
                        "Cannot reach exact zone position, searching for "
            ...
            for (int radius = 1; radius <= 30 && !found_goal; radius++)
            {
                ...
                {
                    GridCell candidate = {goal.first + dx_unit * radius,
                        ...
                        if (!alt_path.empty())
                        {
                            ...
                            geometry_msgs::msg::Point alt_pos =
                                PathPlanner::gridToWorld(*current_map,
                                                         candidate);
                            RCLCPP_INFO(node_->get_logger(),
                                        "Found accessible point near %s at "
                                        "(%.2f, %.2f)",
                                        current_zone.name.c_str(), alt_pos.x,
                                        alt_pos.y);
                            break;
                        }
                    }
...
DeliveryData::DeliveryZone *
DeliveryRobot::findZone(const std::string &zone_name)
{
...
void DeliveryRobot::saveDeliveryRecord(
    const DeliveryData::DeliveryRecord &record)
{
...
void DeliveryRobot::onPointClicked(
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    ...
    response->message = "Started deliveries for " +
                        std::to_string(delivery_queue_.size()) + " requests";
}
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    ...
    try
    {
        ...
        response->success = true;
        response->message = "Saved " + std::to_string(zones_.size()) +
                            " zones to " + zones_file_;
    }
    catch (const std::exception &e)
    {
        response->success = false;
