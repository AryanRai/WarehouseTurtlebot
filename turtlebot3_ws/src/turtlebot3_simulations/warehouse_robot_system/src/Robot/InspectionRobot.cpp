...
InspectionRobot::InspectionRobot(rclcpp::Node::SharedPtr node)
    ...
    if (!check.good()) {
        ...
        log << "Timestamp,RequestID,Site,TagsDetected,Distance(m),Time(s),Status,Notes\n";
        log.close();
    }
    ...
    RCLCPP_INFO(node_->get_logger(), "Tier 1 Safety features ENABLED (TF health + obstacle avoidance)");
...
void InspectionRobot::saveSitesToFile(const std::string& filename) {
    ...
    RCLCPP_INFO(node_->get_logger(), "Saved %zu sites to %s", damage_sites_.size(), filename.c_str());
}
...
void InspectionRobot::startInspections() {
    if (inspection_queue_.empty() && !exploration_mode_) {
        RCLCPP_WARN(node_->get_logger(), "No inspections to execute and exploration mode disabled");
        return;
    }
    ...
    if (exploration_mode_) {
        ...
        if (use_tsp_optimization_) {
            ...
            optimized_route_ = optimizeRouteTSP(current_pose.position, inspection_queue_);
        } else {
            RCLCPP_INFO(node_->get_logger(), "Using ordered inspection sequence");
            optimized_route_ = optimizeRoute(current_pose.position, inspection_queue_);
        }
        
        RCLCPP_INFO(node_->get_logger(), "Starting inspections with %s route of %zu stops",
...
std::vector<InspectionData::DamageSite> InspectionRobot::optimizeRouteTSP(
    ...
    auto optimal_tour = simulatedAnnealing(distance_matrix, 0);  // Start from index 0
...
void InspectionRobot::update() {
    ...
    if (current_inspection_index_ >= optimized_route_.size()) {
        ...
        if (distance_to_home < HOME_TOLERANCE) {
            RCLCPP_INFO(node_->get_logger(), "✓ All inspections completed! Robot at home.");
            
            // Create completion marker file
            std::ofstream marker("/tmp/inspection_complete.marker");
            marker << "Inspection completed at " << getCurrentTimestamp() << std::endl;
            marker.close();
    ...
    double distance_to_site = calculateDistance(current_pose.position, current_site.position);
    
    // Check if we're at the site and need to scan
    if (distance_to_site < TARGET_TOLERANCE) {
        if (!is_scanning_) {
            RCLCPP_INFO(node_->get_logger(), "✓ Reached site: %s - Starting 360° scan", 
        ...
        if (performSiteScan()) {
            ...
            notes << "Inspection completed with " << tags_count << " tags detected";
            if (!detected_tags_.empty()) {
                notes << " (IDs:";
                for (size_t i = 0; i < detected_tags_.size(); ++i) notes << (i ? " " : " ") << detected_tags_[i];
                notes << ")";
            }
            ...
            log << timestamp << ","
                ...
                << site << ","
                << tags_count << ","
                << std::fixed << std::setprecision(3) << distance_m << ","
    ...
    if (distance_to_site < 1.0) {
        ...
        if (has_clear_path) {
            if (!in_target_docking_mode_) {
                RCLCPP_INFO(node_->get_logger(), "Entering precise site docking mode (%.3fm from %s, clear path)", 
            ...
            preciseTargetDocking(current_pose, current_site.position, distance_to_site, current_site.name);
            return;
        } else {
            RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
                                "Close to site (%.2fm) but obstacle detected - continuing with path planner", 
                                distance_to_site);
            in_target_docking_mode_ = false;
        }
    }
    ...
    if (!motion_controller_->hasPath()) {
        RCLCPP_INFO(node_->get_logger(), "Planning path to site: %s", current_site.name.c_str());
        
        GridCell start = PathPlanner::worldToGrid(*current_map, current_pose.position);
        GridCell goal = PathPlanner::worldToGrid(*current_map, current_site.position);
        ...
        RCLCPP_INFO(node_->get_logger(), "Navigating to %s", current_site.name.c_str());
...
void InspectionRobot::onAprilTagDetection(
    ...
    for (const auto& d : msg->detections) {
        ...
        if (!isTagAlreadyDetected(tag_id)) {
            ...
            RCLCPP_INFO(node_->get_logger(), "🏷️  Detected AprilTag ID: %d", tag_id);
        }
    }
}
...
bool InspectionRobot::checkTFHealth() {
    try {
        ...
        if (age > TF_TIMEOUT) {
            RCLCPP_WARN(node_->get_logger(), "TF transform is stale (%.2fs old)", age);
            return false;
        }
        
        return true;
    } catch (const tf2::TransformException& ex) {
        RCLCPP_WARN(node_->get_logger(), "TF health check failed: %s", ex.what());
        return false;
    }
}
...
void InspectionRobot::onLaserScan(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    for (size_t i = front_start; i < front_end; ++i) {
        if (std::isfinite(msg->ranges[i])) {
            min_obstacle_distance_ = std::min(min_obstacle_distance_, static_cast<double>(msg->ranges[i]));
        }
    }
...
bool InspectionRobot::isSafeToMove() {
    // During scanning, we don't check obstacles (robot is stationary and rotating)
    if (is_scanning_) {
        return true;
    }
    ...
    if (obstacle_detected_) {
        RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                            "Obstacle detected at %.2fm - STOPPING", min_obstacle_distance_);
        return false;
    }
...
bool InspectionRobot::isValidPatrolPoint(
    ...
    if (!PathPlanner::isCellInBounds(map, cell) || !PathPlanner::isCellWalkable(map, cell)) {
        return false;
    }
    
    // Check if far enough from walls (base class method)
    double min_dist = checkMinDistanceToWalls(point, map);
    if (min_dist < 0.8) {  // At least 80cm from walls
        return false;
    }
...
void InspectionRobot::executeExploration() {
    if (patrol_points_.empty()) {
        RCLCPP_WARN(node_->get_logger(), "No patrol points available for exploration");
        stopInspections();
        return;
    }
    
    if (current_patrol_index_ >= patrol_points_.size()) {
        RCLCPP_INFO(node_->get_logger(), "✓ Exploration complete - visited all patrol points");
        stopInspections();
        return;
    }
    ...
    if (path.empty()) {
        RCLCPP_WARN(node_->get_logger(), "Cannot reach patrol point %zu, skipping", 
                   current_patrol_index_);
        current_patrol_index_++;
        return;
    }
...
visualization_msgs::msg::Marker InspectionRobot::createSiteMarker(
...
void InspectionRobot::onStartInspectionService(
    ...
    if (inspection_queue_.empty() && !exploration_mode_) {
        ...
        response->message = "No inspection requests in queue and exploration mode disabled";
        return;
    }
    ...
    if (exploration_mode_) {
        ...
        response->message = "Started exploration mode with " + 
                          std::to_string(patrol_points_.size()) + " patrol points";
    } else {
        response->success = true;
        response->message = "Started inspections for " + 
                          std::to_string(inspection_queue_.size()) + " requests";
    }
}
