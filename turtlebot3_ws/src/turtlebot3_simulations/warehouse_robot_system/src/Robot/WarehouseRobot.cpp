...
//              TSP optimization, docking, relocalization, and navigation
//              utilities.
...
WarehouseRobot::WarehouseRobot(rclcpp::Node::SharedPtr node)
    : node_(node), in_docking_mode_(false), in_target_docking_mode_(false),
      initial_yaw_(0.0), has_relocalized_(false),
      relocalization_start_time_(node->now()), use_tsp_optimization_(false),
      path_completed_(false), total_distance_(0.0), battery_level_(100.0f),
      battery_low_threshold_(-1.0), battery_monitoring_enabled_(false),
      low_battery_return_triggered_(false)
{
    ...
    status_pub_ =
        node_->create_publisher<std_msgs::msg::String>("/robot_status", 10);
    ...
    const std::vector<geometry_msgs::msg::Point> &points)
{
    ...
    if (!current_map)
    {
        RCLCPP_ERROR(node_->get_logger(),
                     "No map available for distance matrix calculation!");
        return matrix;
    }
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        {
            GridCell start_cell =
                PathPlanner::worldToGrid(*current_map, points[i]);
            GridCell goal_cell =
                PathPlanner::worldToGrid(*current_map, points[j]);
            ...
            {
                // If no path found, use Euclidean distance as fallback (with
                // penalty)
                double dx = points[j].x - points[i].x;
                double dy = points[j].y - points[i].y;
                double euclidean = std::sqrt(dx * dx + dy * dy);
                matrix[i][j] =
                    euclidean * 10.0; // Heavy penalty for unreachable
                matrix[j][i] = euclidean * 10.0;

                RCLCPP_WARN(node_->get_logger(),
                            "No A* path found between points %zu and %zu, "
                            "using penalized Euclidean distance",
                            i, j);
            }
        }
    }
...
std::vector<int> WarehouseRobot::simulatedAnnealing(
    const std::vector<std::vector<double>> &distance_matrix, int start_idx,
    double initial_temp, double cooling_rate, int max_iterations)
{
    ...
    if (elapsed < 0.1)
    {
        ...
        if (current_map)
        {
            double min_distance_to_wall =
                checkMinDistanceToWalls(current_pose.position, *current_map);

            if (min_distance_to_wall < 0.5)
            { // Less than 50cm from wall
                RCLCPP_WARN(node_->get_logger(),
                            "Too close to wall (%.2fm) for safe relocalization "
                            "spin - skipping",
...
void WarehouseRobot::returnToHome()
{
    ...
    if (distance_to_home < HOME_TOLERANCE)
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "✓ At home position (%.3fm, aligned).",
                    distance_to_home);
        return;
    }
    ...
    if (distance_to_home < DOCKING_DISTANCE)
    {
        ...
        bool has_clear_path =
            hasLineOfSight(current_pose.position, home_position, *current_map);
    ...
    RCLCPP_INFO(node_->get_logger(), "Planning path home (%.2fm)...",
                distance_to_home);
...
void WarehouseRobot::preciseTargetDocking(
    ...
    const geometry_msgs::msg::Point &target_position, double distance_to_target,
...
bool WarehouseRobot::hasLineOfSight(const geometry_msgs::msg::Point &from,
    ...
    const nav_msgs::msg::OccupancyGrid &map)
{
    ...
    for (const auto &[dx, dy] : directions)
    {
        ...
        { // Max 100 cells (~5m at 0.05m resolution)
            GridCell check_cell = {center.first + dx * step,
                                   center.second + dy * step};
            ...
            if (!PathPlanner::isCellWalkable(map, check_cell))
            {
                ...
                geometry_msgs::msg::Point obstacle_pos =
                    PathPlanner::gridToWorld(map, check_cell);
...
void WarehouseRobot::initializeBatteryMonitoring(double threshold)
{
    ...
    {
        RCLCPP_INFO(node_->get_logger(),
                    " Battery monitoring ENABLED - Low threshold: %.1f%%",
                    threshold);
        ...
        battery_sub_ =
            node_->create_subscription<sensor_msgs::msg::BatteryState>(
                "/battery_state", 10,
                std::bind(&WarehouseRobot::onBatteryState, this,
                          std::placeholders::_1));
    }
    else
    {
...
void WarehouseRobot::onBatteryState(
...
bool WarehouseRobot::checkLowBattery()
{
    ...
    {
        RCLCPP_WARN(node_->get_logger(),
                    "️ LOW BATTERY DETECTED! (%.1f%% < %.1f%%)", battery_level_,
                    battery_low_threshold_);
        RCLCPP_WARN(node_->get_logger(),
...
void WarehouseRobot::emergencyReturnHome()
{
    ...
    RCLCPP_WARN(node_->get_logger(),
                "Emergency return home triggered - returning to base");
...
double
WarehouseRobot::calculateDistance(const geometry_msgs::msg::Point &p1,
