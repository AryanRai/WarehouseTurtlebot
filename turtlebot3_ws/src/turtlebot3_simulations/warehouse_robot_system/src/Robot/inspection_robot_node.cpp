...
//
// Description: Implementation of inspection robot node for damage site
// inspection
...
void signalHandler(int signum)
{
    if (g_robot)
    {
        RCLCPP_INFO(rclcpp::get_logger("inspection_robot"),
                    "Interrupt signal (%d) received. Stopping inspections...",
                    signum);
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(node->get_logger(),
                "  Damage Detection with Camera & AprilTags");
    RCLCPP_INFO(node->get_logger(),
                "===========================================");
    RCLCPP_INFO(node->get_logger(), " ");
    RCLCPP_INFO(node->get_logger(), "Setup Instructions:");
    RCLCPP_INFO(node->get_logger(),
                "1. Click points in RViz to define damage sites");
    RCLCPP_INFO(node->get_logger(), "2. Call service: ros2 service call "
                                    "/save_damage_sites std_srvs/srv/Trigger");
    RCLCPP_INFO(node->get_logger(),
                "3. Add inspection requests via code or service");
    RCLCPP_INFO(node->get_logger(), "4. Call service: ros2 service call "
                                    "/start_inspections std_srvs/srv/Trigger");
    RCLCPP_INFO(node->get_logger(), " ");
    ...
    if (mode_env != nullptr)
    {
        ...
        if (mode == "exploration")
        {
            ...
            RCLCPP_INFO(node->get_logger(),
                        " Mode: Inspection Exploration (Systematic Patrol)");
            RCLCPP_INFO(
                node->get_logger(),
                "   Will patrol all accessible areas to discover AprilTags");
        }
        else
        {
            RCLCPP_INFO(node->get_logger(),
                        " Mode: Inspection (Visit Pre-defined Sites)");
        }
    }
    else
    {
        RCLCPP_INFO(node->get_logger(),
                    " Mode: Inspection (Visit Pre-defined Sites) - Default");
    }
    ...
    else
    {
        ...
        if (!exploration_mode)
        {
            RCLCPP_INFO(
                node->get_logger(),
                "   Set INSPECTION_OPTIMIZATION=tsp for optimized routing");
        }
    }
    ...
    // Generate inspection requests for all damage sites (only in inspection
    // mode)
    if (!exploration_mode)
    {
        ...
        if (!sites.empty())
        {
            ...
            RCLCPP_INFO(node->get_logger(),
                        "Added %zu inspection requests for all damage sites",
                        sites.size());
        }
        else
        {
            RCLCPP_WARN(node->get_logger(),
                        "No damage sites defined. Please define sites first.");
        }
    }
            ...
            if (!started)
            {
                ...
                if (g_robot && g_robot->hasValidMap())
                {
                    if (exploration_mode)
                    {
                        RCLCPP_INFO(
                            node->get_logger(),
                            "Map is ready! Starting exploration patrol...");
                    ...
                    else
                    {
                        RCLCPP_INFO(
                            node->get_logger(),
                            "Map is ready! Auto-starting inspections...");
                        g_robot->startInspections();
                    }
                    started = true;
                }
