...
//
// Description: Implementation of autonomous SLAM exploration node for warehouse
// robot.
...
void signalHandler(int signum)
{
    if (g_robot)
    {
        RCLCPP_INFO(rclcpp::get_logger("autonomous_slam"),
                    "Interrupt signal (%d) received. Stopping exploration...",
                    signum);
...
void batteryCallback(const sensor_msgs::msg::BatteryState::SharedPtr msg)
{
    ...
    if (g_auto_return_enabled && g_battery_level < g_battery_return_threshold &&
        g_robot && g_robot->isExploring())
    {
        RCLCPP_WARN(g_node->get_logger(),
                    "Low battery detected (%.1f%% < %.1f%%)! Initiating return "
                    "to home...",
                    g_battery_level, g_battery_return_threshold);
        g_robot->pauseExploration();
        // The robot will automatically return home through the exploration
        // complete logic
    }
}
...
int main(int argc, char **argv)
{
    ...
    g_auto_return_enabled =
        g_node->get_parameter("auto_return_home_enabled").as_bool();
    g_battery_return_threshold =
        g_node->get_parameter("auto_return_threshold_pct").as_double();

    RCLCPP_INFO(g_node->get_logger(), "Battery Auto-Return: %s",
                g_auto_return_enabled ? "ENABLED" : "DISABLED");
    if (g_auto_return_enabled)
    {
        RCLCPP_INFO(g_node->get_logger(), "Return threshold: %.1f%%",
                    g_battery_return_threshold);
    }

    // Create battery subscriber (BatteryState message)
    auto battery_sub =
        g_node->create_subscription<sensor_msgs::msg::BatteryState>(
    ...
    RCLCPP_INFO(g_node->get_logger(),
                "Return home service available at /return_home");
    ...
    RCLCPP_INFO(g_node->get_logger(),
                "Waiting for SLAM Toolbox to initialize...");
    RCLCPP_INFO(g_node->get_logger(),
                "This may take 15-30 seconds for SLAM to build initial map...");
    ...
    while (rclcpp::ok())
    {
        ...
        if (!exploration_finished && g_robot->isExplorationComplete())
        {
            ...
            {
                marker_file
                    << "Exploration completed at: " << std::time(nullptr)
                    << std::endl;
                marker_file.close();
                RCLCPP_INFO(g_node->get_logger(),
                            "Created completion marker for mode switching");
            }
            ...
            RCLCPP_INFO(g_node->get_logger(),
                        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            RCLCPP_INFO(g_node->get_logger(),
                        " EXPLORATION COMPLETE - READY FOR MODE SWITCH");
            RCLCPP_INFO(g_node->get_logger(),
                        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            RCLCPP_INFO(g_node->get_logger(), "");
            RCLCPP_INFO(g_node->get_logger(),
                        "Node staying alive for mode transition...");
            RCLCPP_INFO(g_node->get_logger(),
            ...
            RCLCPP_INFO(g_node->get_logger(),
                        "Exiting to trigger mode selection...");
