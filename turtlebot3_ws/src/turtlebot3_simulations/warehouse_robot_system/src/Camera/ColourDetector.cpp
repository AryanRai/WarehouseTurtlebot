...
//
// Description: Implementation of colour detector node for damage site
// classification
    ...
    if (mCalibrationMode)
    {
        ...
        try
        {
            ...
            if (display == nullptr || strlen(display) == 0)
            {
                RCLCPP_WARN(
                    GetLogger(),
                    "No DISPLAY environment variable - use 'ssh -X' for GUI");
                RCLCPP_INFO(GetLogger(),
                            "Running in FILE mode - saving frames to /tmp/");
            }
            else
            {
                cv::namedWindow(mCalibrationWindowName, cv::WINDOW_AUTOSIZE);
                RCLCPP_INFO(GetLogger(),
                            " CALIBRATION mode - Live camera window enabled");
                RCLCPP_INFO(
                    GetLogger(),
                    " Controls: 's'=save, 'c'=capture values, 'q'=quit");
                ...
                RCLCPP_INFO(GetLogger(), "3. Watch the colored rectangles "
                                         "appear around detected tag");
                RCLCPP_INFO(GetLogger(),
        ...
        catch (const std::exception &aEx)
        {
            RCLCPP_WARN(GetLogger(),
                        "Failed to create window: %s - using file mode",
                        aEx.what());
        }
    }
                                   ...
                                   const rclcpp::Time &aTimestamp)
{
    ...
    if (mCalibrationMode)
    {
        ...
        {
            RCLCPP_WARN(GetLogger(), "️ Calibration mode requires DISPLAY - "
                                     "running headless, GUI disabled");
        ...
        if (has_display)
        {
            ...
            { // Only update display every 3rd frame
                ...
                {
                    // No tags detected yet; still show raw image to confirm
                    // feed
                    try
                    {
                        ...
                        {
                            cv::namedWindow(mCalibrationWindowName,
                                            cv::WINDOW_AUTOSIZE);
                    ...
                    catch (const std::exception &aEx)
                    {
                        ...
                        std::string filename =
                            "/tmp/raw_frame_" +
                            std::to_string(raw_frame_count++) + ".jpg";
                        cv::imwrite(filename, mLatestImage);
                        RCLCPP_INFO_THROTTLE(GetLogger(), *this->get_clock(),
                                             5000, "GUI failed, saving to: %s",
                                             filename.c_str());
                    }
                }
            }
    ...
    const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr aMsg)
{
    ...
    if (mCalibrationMode)
    {
        // In calibration mode, just update the display (no publishing of
        // damage) Only show GUI if DISPLAY is available
        static bool has_display = (std::getenv("DISPLAY") != nullptr);
        if (has_display)
        {
            DisplayCalibrationWindow(mLatestImage, mLastDetections);
    ...
    bool should_display =
        (normal_frame_skip % 3 == 0); // Display every 3rd frame
    ...
    for (const auto &detection : aMsg->detections)
    {
        ...
        if (should_display && !display_image.empty())
        {
            ...
            {
                corners.push_back(cv::Point(static_cast<int>(corner.x),
                                            static_cast<int>(corner.y)));
            }

            for (size_t i = 0; i < corners.size(); ++i)
            {
                cv::line(display_image, corners[i],
                         corners[(i + 1) % corners.size()],
                         cv::Scalar(255, 0, 0),
                         2); // Blue border for normal mode
            }
        }
        ...
        if (should_display && !display_image.empty())
        {
            ...
            std::vector<cv::Rect> samplingRegions =
                ExtractSamplingRegions(centerX, centerY, tagSize,
                                       display_image.cols, display_image.rows);

            // Analyze each region and draw with detected color
            const std::vector<std::string> regionLabels = {"TOP", "BOTTOM",
                                                           "LEFT", "RIGHT"};

            for (size_t i = 0;
                 i < samplingRegions.size() && i < regionLabels.size(); ++i)
            {
                ...
                default:
                    boxColor =
                        cv::Scalar(128, 128, 128); // Gray for no detection
                    colorLabel = "NONE";
                    break;
                }
    ...
    // Show detection window (only if DISPLAY is available) - only on display
    // frames
    if (should_display && !display_image.empty())
    {
        // Check if we have a display (not running headless over SSH)
        ...
        if (has_display)
        {
            ...
            catch (const std::exception &ex)
            {
                ...
                {
                    RCLCPP_WARN(GetLogger(),
                                "Display not available, running headless");
    ...
    for (const auto &detection : aMsg->detections)
    {
        ...
        if (damageType != DAMAGE_NONE && damageType != DAMAGE_UNKNOWN)
        {
            ...
            sReport.mLocalPose.orientation.w =
                1.0; // No rotation data available
            ...
            RCLCPP_INFO(GetLogger(), "Detected %s damage at tag ID %d",
                        DamageTypeToString(damageType).c_str(), detection.id);
        }
    }
}
                                                              ...
                                                              int aImageHeight)
{
    ...
    double scale_factor = std::max(
        0.5, std::min(2.0, aTagSize / 80.0)); // Scale relative to 80px baseline
    ...
    if (topX >= 0 && topY >= 0 && topX + adaptive_width <= aImageWidth &&
        topY + adaptive_height <= aImageHeight)
    {
        regions.push_back(
            cv::Rect(topX, topY, adaptive_width, adaptive_height));
    }
    ...
    {
        regions.push_back(
            cv::Rect(bottomX, bottomY, adaptive_width, adaptive_height));
    }

    // Region left of tag
    int leftX = aTagCenterX - halfSize - adaptive_offset - adaptive_width;
    int leftY = aTagCenterY - adaptive_height / 2;
    if (leftX >= 0 && leftY >= 0 && leftX + adaptive_width <= aImageWidth &&
        leftY + adaptive_height <= aImageHeight)
    {
        regions.push_back(
            cv::Rect(leftX, leftY, adaptive_width, adaptive_height));
    }

    // Region right of tag
    int rightX = aTagCenterX + halfSize + adaptive_offset;
    int rightY = aTagCenterY - adaptive_height / 2;
    if (rightX >= 0 && rightY >= 0 && rightX + adaptive_width <= aImageWidth &&
        rightY + adaptive_height <= aImageHeight)
    {
        regions.push_back(
            cv::Rect(rightX, rightY, adaptive_width, adaptive_height));
    }
...
void CColourDetector::PublishDamageReport(const SDamageReport &aReport)
{
    ...
    jsonStream << "\"damage_type\": \""
               << DamageTypeToString(aReport.mDamageType) << "\", ";
    ...
    RCLCPP_DEBUG(GetLogger(), "Published damage report: %s",
    ...
    const std::vector<apriltag_msgs::msg::AprilTagDetection> &aDetections)
{
    ...
    const double kMinConfidence =
        0.5; // 50% confidence threshold for calibration too
    ...
    for (const auto &detection : aDetections)
    {
        ...
        cv::Point text_pos(centerX - text_size.width / 2,
                           centerY - tagSize / 2 - 10);
        ...
        const std::vector<std::string> labels = {"ABOVE", "BELOW", "LEFT",
                                                 "RIGHT"};
        ...
        {
            cv::circle(annotated, cv::Point(centerX, centerY), 3,
    ...
    if (has_display)
    {
        try
        {
            // Window should already be created in constructor, but ensure it
            // exists
    ...
    if (!has_display)
    {
        ...
        {
            RCLCPP_INFO(GetLogger(),
                        "Running headless - calibration frames saved to /tmp/");
            save_warning_logged = true;
        }
        std::string filename =
            "/tmp/calibration_frame_" + std::to_string(frame_count++) + ".jpg";
                                          ...
                                          int aTagCenterY, int aTagSize)
{
    ...
    {
        if (roi.x < 0 || roi.y < 0 || roi.x + roi.width > aImage.cols ||
        ...
        cv::putText(aImage, label, cv::Point(roi.x, roi.y - 5),
                                           ...
                                           const std::string &aLabel)
{
    ...
    double minH = 0.0, maxH = 0.0, minS = 0.0, maxS = 0.0, minV = 0.0,
           maxV = 0.0;
    if (channels.size() == 3)
    {
    ...
    ss << aLabel << " [" << aRegion.width << "x" << aRegion.height << "px]"
       << std::endl;
    ss << "HSV: [" << static_cast<int>(std::round(meanHSV[0])) << "±"
       << static_cast<int>(maxH - minH) << ", "
       << static_cast<int>(std::round(meanHSV[1])) << "±"
       << static_cast<int>(maxS - minS) << ", "
       << static_cast<int>(std::round(meanHSV[2])) << "±"
       << static_cast<int>(maxV - minV) << "]";
    ...
    else if (aLabel.find("LEFT") != std::string::npos ||
             aLabel.find("RIGHT") != std::string::npos)
    {
        color_instruction = " Place RED object here (blood)";
    }

    // Calculate text placement (above the region with black background for
    // readability)
    std::vector<std::string> lines = {ss.str(), color_instruction};
    int line_height = 15;
    int start_y = std::max(
        15, aRegion.y - (static_cast<int>(lines.size()) * line_height) - 5);
    ...
    for (size_t i = 0; i < lines.size(); ++i)
    {
        ...
        cv::Size text_size = cv::getTextSize(lines[i], cv::FONT_HERSHEY_SIMPLEX,
                                             0.4, 1, nullptr);
...
void CColourDetector::HandleKeyboardInput(int aKeyCode)
{
    ...
    else if (aKeyCode == 'c' || aKeyCode == 'C')
    {
        ...
        {
            RCLCPP_WARN(GetLogger(),
                        " No image available - ensure camera is running");
            return;
        }
        if (mLastDetections.empty())
        {
            RCLCPP_WARN(GetLogger(),
                        " No AprilTags detected - place tag in camera view");
            return;
        }
        ...
        RCLCPP_INFO(
            GetLogger(), "   GREEN (Mould):  H=%d-%d, S=%d-%d, V=%d-%d",
            (int)kGreenLower[0], (int)kGreenUpper[0], (int)kGreenLower[1],
            (int)kGreenUpper[1], (int)kGreenLower[2], (int)kGreenUpper[2]);
        RCLCPP_INFO(GetLogger(),
                    "   BLUE (Water):   H=%d-%d, S=%d-%d, V=%d-%d",
                    (int)kBlueLower[0], (int)kBlueUpper[0], (int)kBlueLower[1],
                    (int)kBlueUpper[1], (int)kBlueLower[2], (int)kBlueUpper[2]);
        RCLCPP_INFO(GetLogger(),
                    "   RED (Blood):    H=%d-%d OR %d-%d, S=%d-%d, V=%d-%d",
                    (int)kRedLower1[0], (int)kRedUpper1[0], (int)kRedLower2[0],
                    (int)kRedUpper2[0], (int)kRedLower1[1], (int)kRedUpper1[1],
        ...
        for (const auto &det : mLastDetections)
        {
            ...
            try
            {
                ...
                if (!c.empty())
                {
                    ...
                    for (const auto &p : c)
                    {
                        cornersPx.emplace_back(static_cast<float>(p.x),
            ...
            for (const auto &pt : cornersPx)
            {
                sx += pt.x;
                sy += pt.y;
            }
            int cx =
                static_cast<int>(sx / static_cast<double>(cornersPx.size()));
            int cy =
                static_cast<int>(sy / static_cast<double>(cornersPx.size()));
            ...
            int tagSize = static_cast<int>(
                std::max(std::max(d01, d12), std::max(d23, d30)));
            ...
            {
                if (r.x < 0 || r.y < 0 || r.x + r.width > mLatestImage.cols ||
            ...
            cv::Scalar mAbove(0, 0, 0, 0), mBelow(0, 0, 0, 0),
                mLeft(0, 0, 0, 0), mRight(0, 0, 0, 0);
            if (regions.size() >= 4)
            {
            ...
            RCLCPP_INFO(GetLogger(), "   ABOVE (place GREEN here):  HSV %s",
                        fmtHSV(mAbove).c_str());
            RCLCPP_INFO(GetLogger(), "   BELOW (place BLUE here):   HSV %s",
                        fmtHSV(mBelow).c_str());
            RCLCPP_INFO(GetLogger(), "   LEFT  (place RED here):    HSV %s",
                        fmtHSV(mLeft).c_str());
            RCLCPP_INFO(GetLogger(), "   RIGHT (place RED here):    HSV %s",
                        fmtHSV(mRight).c_str());
            ...
            RCLCPP_INFO(GetLogger(), "3. When satisfied with detection, press "
...
void CColourDetector::SaveCalibrationToYAML(const std::string &aFilePath)
{
    ...
    try
    {
        ...
        if (!ofs.is_open())
        {
            RCLCPP_ERROR(GetLogger(), "Failed to open file for writing: %s",
                         path.c_str());
            return;
        }
    ...
    {
        RCLCPP_ERROR(GetLogger(), "Exception while saving YAML: %s",
                     aEx.what());
    }
}
