...
//
// Description: Implementation of unified camera node that subscribes to
// multiple
...
void CameraNode::LoadCameraTopics()
{
    ...
    else
    {
        ...
        paramTopics = this->declare_parameter<std::vector<std::string>>(
            "camera_topics", std::vector<std::string>{"/camera/image_raw"});
    }
    ...
    if (paramTopics.empty())
    {
        RCLCPP_WARN(this->get_logger(),
                    "No camera topics specified, using default");
        mCameraTopics.push_back("/camera/image_raw");
    }
    else
    {
        mCameraTopics = paramTopics;
        RCLCPP_INFO(this->get_logger(), "Loaded %zu camera topics",
                    mCameraTopics.size());
