...
std::pair<FrontierList, std::vector<GridCell>> FrontierSearch::search(
    ...
    while (!queue.empty()) {
        ...
        // Check ALL neighbors (including unknown ones) - must_be_walkable = false
        for (const auto& neighbor : PathPlanner::neighborsOf4(mapdata, current, false)) {
...
std::pair<Frontier, std::vector<GridCell>> FrontierSearch::buildNewFrontier(
    ...
    while (!queue.empty()) {
        ...
        for (const auto& neighbor : PathPlanner::neighborsOf8(mapdata, current, false)) {
