...
void AutonomousSlamController::handleSearchingFrontiers() {
    ...
    if (!searchForFrontiers()) {
        ...
        RCLCPP_INFO(this->get_logger(), "No frontiers found (count: %d/%d)", 
                    consecutive_no_frontiers_, config_.max_no_frontier_count);
        
        // Only do area exploration if we've consistently had no frontiers
        if (consecutive_no_frontiers_ >= 3 && consecutive_no_frontiers_ < config_.max_no_frontier_count) {
            RCLCPP_INFO(this->get_logger(), "Performing quick scan for hidden frontiers...");
            transitionToMappingState(MappingState::EXPLORING_AREA);
            return;
        }
        
        // Only declare complete after many consecutive failures
        if (consecutive_no_frontiers_ >= config_.max_no_frontier_count) {
            RCLCPP_INFO(this->get_logger(), "Exploration complete - no frontiers after %d checks!", 
                        config_.max_no_frontier_count);
...
void AutonomousSlamController::handleExploringArea() {
    // Quick scan instead of full rotation - just look around briefly
    static rclcpp::Time rotation_start = this->now();
    static int scan_count = 0;
    
    const double rotation_duration = 1.0; // seconds - REDUCED from 2.0 for quicker scans
    const double rotation_speed = 0.8; // rad/s - faster rotation for quick look around
    ...
    RCLCPP_INFO(this->get_logger(), "Quick scan %d complete, resuming frontier search", scan_count);
    
    // Don't increment no_frontiers counter - let the frontier search do that
    // This prevents premature completion when we're just doing a quick look
...
void AutonomousSlamController::calculatePurePursuitControl(const geometry_msgs::msg::Point& lookahead) {
    ...
    // MORE CONSERVATIVE adaptive speed reduction for sharp turns (prevents tipping)
    double abs_alpha = std::abs(alpha);
    if (abs_alpha > 1.4) {  // Very sharp turn (>80°)
        linear_vel *= 0.15;  // Drastically slow down
        angular_vel *= 0.7;  // Also reduce angular velocity to prevent tipping
    } else if (abs_alpha > 1.0) {  // Sharp turn (>57°)
        linear_vel *= 0.25;
        angular_vel *= 0.8;
    } else if (abs_alpha > 0.7) {  // Moderate turn (>40°)
        linear_vel *= 0.35;
    } else if (abs_alpha > 0.5) {  // Medium turn (>28°)
        linear_vel *= 0.5;
