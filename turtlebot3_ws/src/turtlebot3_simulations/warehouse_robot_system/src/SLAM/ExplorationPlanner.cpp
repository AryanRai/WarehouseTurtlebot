...
ExplorationPlanner::ExplorationPlanner()
    ...
    exploration_status_sub_ = this->create_subscription<std_msgs::msg::Bool>(
        ...
        std::bind(&ExplorationPlanner::explorationStatusCallback, this, std::placeholders::_1)
    );
    ...
    // Path planner reference (will be set externally if needed for direct calls)
    // Or use topic-based communication
    ...
    RCLCPP_INFO(this->get_logger(), "  Exploration rate: %.1f Hz", EXPLORATION_RATE);
}
...
void ExplorationPlanner::explorationStatusCallback(const std_msgs::msg::Bool::SharedPtr msg) {
    if (msg->data && !is_finished_exploring_) {
        RCLCPP_INFO(this->get_logger(), "Exploration marked complete by SLAM controller");
...
void ExplorationPlanner::planExplorationPathInternal() {
    ...
    GridCell start = PathPlanner::worldToGrid(*current_map_, current_pose_.position);
    
    // Search for frontiers
    auto [frontier_list, frontier_cells] = FrontierSearch::search(*current_map_, start, debug_mode_);
    
    // Publish frontier cells for visualization
    if (debug_mode_ && frontier_cells_pub_) {
        auto grid_cells = PathPlanner::getGridCells(*current_map_, frontier_cells);
    ...
    if (frontier_list.frontiers.empty()) {
        ...
        if (no_frontiers_found_counter_ >= NUM_EXPLORE_FAILS_BEFORE_FINISH) {
            RCLCPP_INFO(this->get_logger(), "Exploration complete - no more frontiers");
            is_finished_exploring_ = true;
    ...
    RCLCPP_INFO(this->get_logger(), "Evaluating %zu frontiers", top_frontiers.size());
    ...
    for (const auto& frontier : top_frontiers) {
        ...
        if (distance_to_frontier < GOAL_TOLERANCE) {
            RCLCPP_DEBUG(this->get_logger(), 
                        "Skipping frontier - within goal tolerance (%.3fm)", distance_to_frontier);
            continue;
        }
        ...
        GridCell start = PathPlanner::worldToGrid(*current_map_, current_pose_.position);
        GridCell goal = PathPlanner::worldToGrid(*current_map_, frontier.centroid);
        ...
        if (!path_cells.empty() && a_star_cost > 0.0) {
            ...
            if (cost < lowest_cost) {
                lowest_cost = cost;
                best_path = PathPlanner::pathToMessage(*current_map_, path_cells);
    ...
    if (found_valid_path) {
        ...
        if (goal_distance < GOAL_TOLERANCE) {
            ...
            RCLCPP_INFO(this->get_logger(), "Publishing path with cost %.2f", lowest_cost);
        ...
        if (frontiers_checked == 0 && frontiers_too_close > 0) {
            ...
            if (no_path_found_counter_ >= completion_threshold) {
                RCLCPP_INFO(this->get_logger(), "Exploration complete - no valid paths");
                is_finished_exploring_ = true;
