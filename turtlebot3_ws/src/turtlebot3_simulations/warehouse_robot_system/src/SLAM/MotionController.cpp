...
MotionController::MotionController()
    : Node("motion_controller"), has_path_(false), has_valid_map_(false),
      has_valid_pose_(false), enabled_(true), reversed_(false), alpha_(0.0),
      ...
      max_turn_speed_(INSPECTION_ANGULAR_SPEED)
{
    ...
    path_sub_ = this->create_subscription<nav_msgs::msg::Path>(
        "/motion/path", 10,
        std::bind(&MotionController::pathCallback, this,
                  std::placeholders::_1));
    ...
    pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/slam/pose", 10,
        std::bind(&MotionController::poseCallback, this,
                  std::placeholders::_1));

    speed_profile_sub_ = this->create_subscription<std_msgs::msg::String>(
        "/motion/speed_profile", 10,
        std::bind(&MotionController::speedProfileCallback, this,
                  std::placeholders::_1));
    ...
    goal_reached_pub_ =
        this->create_publisher<std_msgs::msg::Bool>("/motion/goal_reached", 10);

    if (debug_mode_)
    {
        fov_cells_pub_ = this->create_publisher<nav_msgs::msg::GridCells>(
            "/motion/fov_cells", 10);
        close_wall_cells_pub_ =
            this->create_publisher<nav_msgs::msg::GridCells>(
                "/motion/close_wall_cells", 10);
    }

    // No service in simplified version - use setEnabled() method directly if
    // needed
...
void MotionController::pathCallback(const nav_msgs::msg::Path::SharedPtr msg)
{
    ...
    if (has_path_)
    {
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "New path received with %zu waypoints, goal at (%.2f, %.2f)",
            msg->poses.size(), goal.x, goal.y);
    }
}

void MotionController::mapCallback(
...
void MotionController::poseCallback(
...
void MotionController::speedProfileCallback(
    const std_msgs::msg::String::SharedPtr msg)
{
    ...
    {
        RCLCPP_WARN(this->get_logger(), "Unknown speed profile: %s",
                    profile.c_str());
    }
}
...
double
MotionController::getDistanceToWaypoint(const geometry_msgs::msg::Pose &pose,
                                        int index) const
{
...
int MotionController::findNearestWaypointIndex(
...
MotionController::findLookahead(const geometry_msgs::msg::Pose &pose,
    ...
    const geometry_msgs::msg::Pose &pose)
{
    ...
    // Get robot grid cell
    GridCell robot_cell =
        PathPlanner::worldToGrid(*current_map_, pose.position);
    ...
    for (int dx = -FOV_DISTANCE; dx <= FOV_DISTANCE; dx++)
    {
        for (int dy = -FOV_DISTANCE; dy <= FOV_DISTANCE; dy++)
        {
            ...
            // Check if in FOV
            bool is_in_fov =
                (dist <= FOV_DISTANCE && angle >= -FOV / 2.0 &&
                 angle <= FOV / 2.0 && std::abs(angle) >= FOV_DEADZONE / 2.0);
            bool is_in_small_fov =
                (dist <= SMALL_FOV_DISTANCE && angle >= -SMALL_FOV / 2.0 &&
                 angle <= SMALL_FOV / 2.0);
    ...
    if (debug_mode_)
    {
        ...
        {
            auto grid_cells =
                PathPlanner::getGridCells(*current_map_, fov_cells);
        ...
        {
            auto grid_cells =
                PathPlanner::getGridCells(*current_map_, wall_cells);
            grid_cells.header.stamp = this->now();
...
geometry_msgs::msg::Twist MotionController::computeVelocityCommand()
{
    ...
    geometry_msgs::msg::Point lookahead =
        findLookahead(current_pose_, nearest_idx);
    ...
    // Check if at goal
    double distance_to_goal = distance(
        current_pose_.position.x, current_pose_.position.y, goal.x, goal.y);
    if (distance_to_goal < DISTANCE_TOLERANCE)
    {
        RCLCPP_INFO(this->get_logger(), "Reached goal! Distance: %.3fm",
                    distance_to_goal);
