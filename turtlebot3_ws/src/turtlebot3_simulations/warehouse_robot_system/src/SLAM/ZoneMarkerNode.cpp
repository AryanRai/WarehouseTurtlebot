...
// File: ZoneMarkerNode.cpp
// Author(s): Aryan Rai
//
// Description: Implementation of ZoneMarkerNode class

#include "SLAM/ZoneMarkerNode.hpp"
#include <fstream>
#include <yaml-cpp/yaml.h>

ZoneMarkerNode::ZoneMarkerNode() : Node("zone_marker_node")
{
    // Subscribe to clicked points
    clicked_point_sub_ =
        this->create_subscription<geometry_msgs::msg::PointStamped>(
            "/clicked_point", 10,
            std::bind(&ZoneMarkerNode::onPointClicked, this,
                      std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(),
                "Publishing markers to: /delivery_zones/markers");
    RCLCPP_INFO(this->get_logger(),
                "Click points in RViz to add delivery zones");
    RCLCPP_INFO(this->get_logger(), "Currently have %zu zones",
                zones_.size());
    ...
    if (!zones_.empty())
    {
        publishMarkers();
        RCLCPP_INFO(this->get_logger(),
                    "Published %zu existing zone markers",
                    zones_.size());
    }
}

void ZoneMarkerNode::onPointClicked(
    ...
    try
    {
        ...
        if (config["delivery_zones"])
        {
            for (const auto &zone_node : config["delivery_zones"])
            {
                ...
                zone.position.z = 0.0;
                zone.description =
                    zone_node["description"].as<std::string>("");
                zones_.push_back(zone);
            }
        }
    }
...
void ZoneMarkerNode::onClearZones(
...
void ZoneMarkerNode::publishMarkers()
{
    visualization_msgs::msg::MarkerArray marker_array;
    ...
    for (size_t i = 0; i < zones_.size(); i++)
    {
        ...
        marker.lifetime =
            rclcpp::Duration::from_seconds(0); // Persistent
