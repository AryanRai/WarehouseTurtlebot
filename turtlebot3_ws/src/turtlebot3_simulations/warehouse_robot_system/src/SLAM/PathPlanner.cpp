...
static const std::vector<GridCell> DIRECTIONS_OF_4 = {
    {-1, 0}, {1, 0}, {0, -1}, {0, 1}};
...
PathPlanner::PathPlanner() : Node("path_planner"), has_valid_map_(false)
{
    ...
    // Publishers
    path_pub_ =
        this->create_publisher<nav_msgs::msg::Path>("/path_planner/path", 10);
    cspace_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>(
        "/path_planner/cspace", 10);
    cost_map_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>(
        "/path_planner/cost_map", 10);
    path_cells_pub_ = this->create_publisher<nav_msgs::msg::GridCells>(
        "/path_planner/path_cells", 10);
...
void PathPlanner::updateCostMap()
{
    ...
    for (int y = 0; y < current_cost_map_.rows; y++)
    {
        for (int x = 0; x < current_cost_map_.cols; x++)
        {
            ...
            double normalized =
                (current_cost_map_.at<uint8_t>(y, x) / max_val) * 100.0;
...
nav_msgs::msg::Path
PathPlanner::planPath(const geometry_msgs::msg::PoseStamped &start,
                      const geometry_msgs::msg::PoseStamped &goal)
{
    ...
    if (!has_valid_map_ || !current_map_)
    {
        RCLCPP_WARN(this->get_logger(),
                    "No valid map available for path planning");
        return empty_path;
    }
...
int PathPlanner::gridToIndex(const nav_msgs::msg::OccupancyGrid &mapdata,
                             const GridCell &p)
{
    return p.second * mapdata.info.width + p.first;
}

int8_t PathPlanner::getCellValue(const nav_msgs::msg::OccupancyGrid &mapdata,
                                 const GridCell &p)
{
...
PathPlanner::gridToWorld(const nav_msgs::msg::OccupancyGrid &mapdata,
                         const GridCell &p)
{
    geometry_msgs::msg::Point point;
    point.x = (p.first + 0.5) * mapdata.info.resolution +
              mapdata.info.origin.position.x;
    point.y = (p.second + 0.5) * mapdata.info.resolution +
              mapdata.info.origin.position.y;
    point.z = 0.0;
    return point;
}

GridCell PathPlanner::worldToGrid(const nav_msgs::msg::OccupancyGrid &mapdata,
                                  const geometry_msgs::msg::Point &wp)
{
    int x = static_cast<int>((wp.x - mapdata.info.origin.position.x) /
                             mapdata.info.resolution);
    int y = static_cast<int>((wp.y - mapdata.info.origin.position.y) /
                             mapdata.info.resolution);
    return {x, y};
}
...
double PathPlanner::euclideanDistance(const std::pair<double, double> &p1,
...
bool PathPlanner::isCellInBounds(const nav_msgs::msg::OccupancyGrid &mapdata,
                                 const GridCell &p)
{
...
bool PathPlanner::isCellWalkable(const nav_msgs::msg::OccupancyGrid &mapdata,
                                 const GridCell &p)
{
    if (!isCellInBounds(mapdata, p))
        return false;
    return getCellValue(mapdata, p) >= 0 &&
           getCellValue(mapdata, p) < WALKABLE_THRESHOLD;
}

std::vector<GridCell>
PathPlanner::neighborsOf4(const nav_msgs::msg::OccupancyGrid &mapdata,
...
std::vector<GridCell>
PathPlanner::neighborsOf8(const nav_msgs::msg::OccupancyGrid &mapdata,
                          const GridCell &p, bool must_be_walkable)
{
    std::vector<GridCell> neighbors;
...
std::vector<std::pair<GridCell, double>> PathPlanner::neighborsAndDistancesOf8(
    const nav_msgs::msg::OccupancyGrid &mapdata, const GridCell &p,
...
PathPlanner::calcCSpace(const nav_msgs::msg::OccupancyGrid &mapdata,
                        bool include_cells)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            map.at<uint8_t>(y, x) =
                (value == -1) ? 255 : static_cast<uint8_t>(value);
        }
    }
    ...
    cv::Mat kernel = cv::getStructuringElement(
        cv::MORPH_RECT, cv::Size(CSPACE_PADDING, CSPACE_PADDING));
...
cv::Mat
PathPlanner::createHallwayMask(const nav_msgs::msg::OccupancyGrid &mapdata,
...
cv::Mat PathPlanner::calcCostMap(const nav_msgs::msg::OccupancyGrid &mapdata)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            int8_t value = mapdata.data[y * width + x];
            map.at<uint8_t>(y, x) =
                (value == -1) ? 100 : static_cast<uint8_t>(value);
        }
    }
...
GridCell PathPlanner::getFirstWalkableNeighbor(
    const nav_msgs::msg::OccupancyGrid &mapdata, const GridCell &start)
{
...
PathPlanner::aStar(const nav_msgs::msg::OccupancyGrid &mapdata,
                   const cv::Mat &cost_map, const GridCell &start_in,
                   const GridCell &goal_in)
{
    // Get walkable start and goal
    GridCell start = isCellWalkable(mapdata, start_in)
                         ? start_in
                         : getFirstWalkableNeighbor(mapdata, start_in);
    GridCell goal = isCellWalkable(mapdata, goal_in)
                        ? goal_in
                        : getFirstWalkableNeighbor(mapdata, goal_in);
    ...
    while (!pq.empty())
    {
        ...
        for (const auto &[neighbor, distance] :
             neighborsAndDistancesOf8(mapdata, current))
        {
            double added_cost = distance;
            if (!cost_map.empty())
            {
                added_cost +=
                    COST_MAP_WEIGHT * getCostMapValue(cost_map, neighbor);
            }
...
PathPlanner::pathToPoses(const nav_msgs::msg::OccupancyGrid &mapdata,
                         const std::vector<GridCell> &path)
{
    ...
    for (size_t i = 0; i < path.size() - 1; i++)
    {
        ...
        double angle = std::atan2(next_cell.second - cell.second,
                                  next_cell.first - cell.first);
...
nav_msgs::msg::Path
PathPlanner::pathToMessage(const nav_msgs::msg::OccupancyGrid &mapdata,
                           const std::vector<GridCell> &path)
{
...
PathPlanner::getGridCells(const nav_msgs::msg::OccupancyGrid &mapdata,
