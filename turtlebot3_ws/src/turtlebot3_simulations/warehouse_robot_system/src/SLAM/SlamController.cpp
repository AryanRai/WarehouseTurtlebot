...
SlamController::SlamController()
    : Node("slam_controller"), has_valid_map_(false), has_valid_pose_(false),
      exploration_complete_(false)
{
    ...
    // Publishers
    map_pub_ =
        this->create_publisher<nav_msgs::msg::OccupancyGrid>("/slam/map", 10);
    pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
        "/slam/pose", 10);
    map_ready_pub_ =
        this->create_publisher<std_msgs::msg::Bool>("/slam/map_ready", 10);
    ...
    RCLCPP_INFO(this->get_logger(), "  Pose publish rate: %.1f Hz",
                pose_publish_rate_);
    RCLCPP_INFO(this->get_logger(), "  Map republish rate: %.1f Hz",
                map_republish_rate_);
}
...
void SlamController::mapCallback(
...
void SlamController::saveMap(const std::string &map_name)
{
    ...
    {
        pose_file << current_pose_.position.x << " " << current_pose_.position.y
        ...
        pose_file.close();
        RCLCPP_INFO(this->get_logger(), "Robot pose saved to: %s_pose.txt",
                    map_name.c_str());
    }

    RCLCPP_INFO(this->get_logger(),
                "Map save initiated. Files will appear shortly.");
}

void SlamController::setExplorationComplete(bool complete)
{
    exploration_complete_ = complete;
    RCLCPP_INFO(this->get_logger(), "Exploration marked as %s",
