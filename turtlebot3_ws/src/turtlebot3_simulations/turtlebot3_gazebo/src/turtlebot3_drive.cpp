...
// Modified for warehouse_robot_system compatibility
...
: Node("turtlebot3_drive_node")
{
  ...
  ************************************************************/
  ...
  // Declare parameter for autonomous mode
  this->declare_parameter("autonomous_mode", false);
  ...
  ************************************************************/
  // Only enable wall-following if not in autonomous mode
  if (!autonomous_mode_) {
    update_timer_ = this->create_wall_timer(10ms, std::bind(&Turtlebot3Drive::update_callback, this));
    RCLCPP_INFO(this->get_logger(), "Turtlebot3 wall-following node initialized");
  } else {
    RCLCPP_INFO(this->get_logger(), "Turtlebot3 drive node initialized in autonomous mode (wall-following disabled)");
  }
}
